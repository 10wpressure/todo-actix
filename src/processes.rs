...
use crate::to_do::structs::on_hold::OnHold;
...
fn process_pending(item: Pending, command: String, state: &Map<String, Value>) {
    ...
    match command.as_str() {
        ...
        "on_hold" => item.set_to_on_hold(&item.super_struct.title, &mut state),
...
pub fn process_input(item: ItemTypes, command: String, state: &Map<String, Value>) {
    match item {
        ...
        ItemTypes::OnHold(item) => process_on_hold(item, command, state),
    }
}